...
class Transformer:
  ...
  def forward(self, tokens, start_pos, temperature,n_tokens,np_out):
    if start_pos > 0:
      h = openclk.add(self.wte_weight,self.wpe_weight,start_pos,tokens)
      attn_dim = dim
      ...
      tokens = openclk.kernel_1(h,self.ln_f_weight, self.ln_f_bias,self.lm_head_weight,temperature,unif_samples,start_pos,tokens,np_out)
      return tokens
    else:
    ...
    unif_samples = rand.rand()
    tokens = openclk.kernel_3(x,self.ln_1_weight[-1], self.ln_1_bias[-1],self.attn_c_attn_weight[-1],self.attn_c_attn_bias[-1],self.attn_cache_kv[-1]\
    ,self.ln_f_weight, self.ln_f_bias,n_tokens,MAX_CONTEXT,self.lm_head_weight_unf,temperature,unif_samples,tokens)
    return tokens

  def __call__(self, tokens, start_pos, temperature:np.float32,n_tokens,np_out):
    return self.forward(tokens, start_pos, temperature,n_tokens,np_out)
...
class GPT2:
  ...
  def generate(self, prompt:str, max_length:int, temperature:float, timing:bool=False, batch_size:int=1,expected_tokens=None):
    toksb = np.ones(MAX_CONTEXT).astype(np.int32)*-1

    toks = encode(prompt)
    start_pos = 0
    n_tokens = len(encode(prompt))

    toksb[0:n_tokens] = toks
    toksb = cl.Buffer(ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=toksb)
    for i in trange(max_length, disable=(timing==True)):
      np_out = False
      if i == max_length - 1:
        np_out = True
      toksb = self.model(toksb, start_pos, temperature, n_tokens,np_out)
      if start_pos == 0:
        start_pos = len(encode(prompt))
      else:
        start_pos += 1
      #if expected_tokens != None:
      #np.testing.assert_equal(toksb[start_pos],expected_tokens[start_pos-n_tokens])
    return decode(toksb)
