...
class Opencl_Kernels:
    ...
    def add(self,a_g,b_g,b_s,token_g):
        if type(token_g) == np.ndarray:
            token_g = cl.Buffer(ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=token_g)
        ...
        prg_str = f"""
        __kernel void add(
            __global const float *a, __global const float *b, __global const int *token, __global float *res)
        {{
        int gidx0 = get_global_id(0);
        int t = token[{b_s}];
        res[gidx0] = a[t*{self.dim} + gidx0] + b[gidx0 + {b_s*self.dim}];   
        }}
        """
        if prg_str not in self.prg_cache:
            self.prg_cache[prg_str] = cl.Program(ctx,prg_str).build()
        prg = self.prg_cache[prg_str]
        prg.add(queue, (self.dim,1), (256,1), a_g, b_g, token_g, self.add_res_g) #todo check shape
        return self.add_res_g

    def tok_emb(self,tokens_g,weight_g,weight_2_g,no_tokens):
        if type(tokens_g) == np.ndarray:
            tokens_g = cl.Buffer(ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=tokens_g)
        ls = 256
    ...
    def kernel_1(self,h_g,weight_g,bias_g,weight2_g,temperature,random_num,start_pos,tokens_g,np_out):
        ls = 256
        ...
        __kernel void mm12(
        __global const float *a, __global int *tokens)
        {{
            __attribute__ ((aligned (16))) __local float temp[{ls}];
            int lidx0 = get_local_id(0);
            float t = 0;
        ...
        prg.mm12(queue, (ls,1), (ls,1), self.logits_g, tokens_g)
        
        if(np_out):
            print("NP OUT")
            t = np.zeros(113).astype(np.int32)
            print("MADE ARRAY")
            cl.enqueue_copy(queue, t, tokens_g)
            print("COPIED")
            return t
        return tokens_g

    def kernel_3(self,x_g,weight_g,bias_g,attn_weight_g,attn_bias_g,new_cache_g\
        ,ln_f_weight_g,ln_f_bias_g,n_tokens,max_content,lm_head_weight_g,temperature,random_num,tokens_g):
        ls = 256
        ...
        prg.mm13(queue, (ls,1), (ls,1), logits_g, tokens_g)
        return tokens_g
