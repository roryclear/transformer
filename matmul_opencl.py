...
def matmul2(a,b):
    ...
    """).build()
    knl = prg.matmul
    knl(queue, (128,1), (128,1), a_g, b_g,c_g) #shape of output !!!!!
    cl.enqueue_copy(queue, c, c_g)
    return c
...
def matmul2_tg(a,b):
    ...
    for (int ridx0 = 0; ridx0 < 8; ridx0++) {{
        ...
        int alu2 = (alu0+(lidx2*128)+(ridx0*1024));
    ...
    *((__local float4*)(temp+alu1+(lidx2*4))) = (float4)(float4)(acc0,acc1,acc2,acc3);
    ...
    """).build()
    knl = prg.matmul
    knl(queue, (64,4), (8,4), a_g, b_g,c_g) #todo check shape 16*x and 16*1 (DONT CHANGE)
    cl.enqueue_copy(queue, c, c_g)
    return c
...
b = np.random.rand(64,128).astype(np.float32)
c_np = np.matmul(a,b)
