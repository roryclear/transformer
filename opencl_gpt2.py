...
mf = cl.mem_flags
cl_params = params = {"ctx":ctx,"mf":mf,"queue":cl.CommandQueue(ctx)}
  ...
  def forward(self, tokens, start_pos, temperature:float=0.8,n_tokens=444):
    if start_pos > 0:
      ...
      for i in range(0,len(self.ln_1_weight)):
        ...
        self.mlp_c_proj_weight[i],self.mlp_c_proj_bias[i],start_pos,attn_dim)
...
if __name__ == "__main__":
  ...
  excepted_tokens_large = [198, 198, 1532, 345, 550, 257, 40663, 11, 345, 561,
...
345, 561, 7925, 1657, 12, 6551, 5696, 422, 262, 3668]
  ...
  k = kernels.Kernels(dim=768,n_heads=12,max_context=MAX_CONTEXT,device="OpenCL")
  dim = 768
  n_heads = 12
  ...
  MAX_CONTEXT = len(encode(default_prompt))+100
  k = kernels.Kernels(dim=1024,n_heads=16,max_context=MAX_CONTEXT,device="OpenCL")
  dim = 1024
  n_heads = 16
  ...
  dim = 1280
  n_heads = 20
  k = kernels.Kernels(dim=1280,n_heads=20,max_context=MAX_CONTEXT,device="OpenCL")
  ...
  gpt2.model.to_buffer()
  rand = Rand()
  text = gpt2.generate(prompt=default_prompt, max_length=100, temperature=np.float32(0.8), timing=None, batch_size=1,expected_tokens=excepted_tokens_large)
  print((f"Response:", "green"), text)