...
class Opencl_Kernels:
    ...
    def kernel_0_b(self,x,weight_g,bias_g,attn_weight_g,attn_bias_g,new_cache_g\
        ...
        {{
            ...
            int i = gidx0 / {16*64};
            int j = gidx0 % {16*64};
            new_cache[i*16*64 + j] = xqkv[i*16*64*3 + j + 1024];
            new_cache[{max_content}*16*64 + i*16*64 + j] = xqkv[i*16*64*3 + j + 1024*2]; 
        }}
        __kernel void mm5(
        ...
        {{
            ...
            barrier(CLK_LOCAL_MEM_FENCE);
            for(int i = 0; i < {seg}; i++) {{
                x[i + lidx0*{seg} + {(n_tokens - 1)*1024}] -= mean;
            }}
            barrier(CLK_LOCAL_MEM_FENCE);
            total = 0;
            for(int i = 0; i < {seg}; i++) {{
                total += pow(x[lidx0*{seg} + i + {(n_tokens - 1)*1024}],2);
            }}
            temp[lidx0] = total;
            barrier(CLK_LOCAL_MEM_FENCE);
            if(lidx0==0) {{
            ...
            barrier(CLK_LOCAL_MEM_FENCE);
            for(int i = 0; i < {seg}; i++) {{
                x[i + lidx0*{seg}] = (x[i + lidx0*{seg} + {(n_tokens - 1)*1024}] * ln_f_weight[i + lidx0*{seg}]) / mean + ln_f_bias[i + lidx0*{seg}];
            }}
        }}
        """
        if prg_str not in self.prg_cache:
    ...
    def kernel_2(self,a_g,c_g,d_g,e_g,xqkv_g,g,keys_values_g,start_pos,weight_g,bias_g,\
        ...
        {{
            ...
            for(int i = 0; i < {int(dim*3 / ls)}; i++) {{
                ...
                if((lidx0*{int(dim*3 / ls)} + i) >= {g} && (lidx0*{int(dim*3 / ls)} + i) < {2*g}) {{
                    keys_values[{start_pos}*{dim} + lidx0*{int(dim*3 / ls)} + i - {g}] = xqkv[1024 + lidx0*{int(dim*3 / ls)} + i - {g}] + total;
                }}
                if((lidx0*{int(dim*3 / ls)} + i) >= {2*g}) {{
                    keys_values[1024*128 + {start_pos}*{dim} + lidx0*{int(dim*3 / ls)} + i - {2*g}] = xqkv[{1024*2} + lidx0*{int(dim*3 / ls)} + i - {2*g}] + total;
                }}
            }}
        }}
        ...
        {{
            ...
            for(int g = 0; g < {seg}; g++) {{
                ...
                float acc0 = 0;
                for(int i = 0; i < {start_pos+1}; i++) {{
                    acc0 += temp3[i + {start_pos+1}*y] * keys_values[1024*128 + i*16*64 + x + y*64];
                }}
                xq_temp[x + y*64] = acc0;
            }}
            barrier(CLK_LOCAL_MEM_FENCE);
    ...
    def matmul_t_f(self,x_g,attn_weight_g,n_tokens,attn_bias_g):
        ...
        knl = prg.matmul
        knl(queue, (g,1), (ls,1), x_g, attn_weight_g,attn_bias_g,c_g) 
        cl.enqueue_copy(queue, c, c_g)
        return c
    ...
    def copy_to_cache(self,xqkv,new_cache_g,n_tokens,max_content):
        #xk = xk.flatten()
        #xv = xv.flatten()
        xqkv = xqkv.flatten()
    ...
    def matmul_t_3d(self,xq_g,xv_g,n_tokens):
        ...
        """).build()
        g = (math.ceil(16*b_cols*n_tokens / ls) * ls)
        knl = prg.matmul
    ...
    def kernel_7(self,x,ln_1_weight_g,ln_1_bias_g,attn_weight_g,attn_bias_g,cache_kv_g,attn_c_proj_weight_g,attn_c_proj_bias_g,ln_2_weight_g,ln_2_bias_g,c_fc_weight,c_fc_bias_g\
        ...
        x_g = cl.Buffer(ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=x)
        xq = np.zeros(16*64*num_tokens).astype(np.float32) #todo
        xv = np.zeros(16*64*num_tokens).astype(np.float32) #todo
        ...
        {{
            ...
            int k = gidx0 % 64;
            xq[i*{num_tokens}*64 + j*64 + k] = xqkv[i*64 + j*64*16*3 + k];
            xv[i*{num_tokens}*64 + j*64 + k] = xqkv[i*64 + j*64*16*3 + k + 64*16*2];
        }}
        __kernel void ms0(
        ...
        {{
            ...
            if(gidx0 < {n}*{a_rows}*{a_rows}) {{
                ...
                for(int k = 0; k < {a_cols}; k++) {{
                    total += xq[y*{a_cols} + k + z*{a_rows}*{a_cols}] * xqkv[x*64*16*3 + k + z*64 + 1024]; 
                }}
        ...
        {{
            ...
            int j = (gidx0 / 64) % {num_tokens};
            int k = gidx0 % 64;
            xqt[i*64 + j*16*64 + k] = xq[i*{num_tokens}*64 + j*64 + k];
        }}
        __kernel void ms7(
...
__kernel void ms8(
        ...
        {{
            ...
            barrier(CLK_LOCAL_MEM_FENCE);
            for(int i = 0; i < {seg}; i++) {{
                x[1024*r + i + lidx0*{seg}] = (x[1024*r + i + lidx0*{seg}] * ln_2_weight[i + lidx0*{seg}]) / temp2[r] + ln_2_bias[i + lidx0*{seg}];
            }}
        }}
        __kernel void ms9(
    ...
    def transpose(self,xq_g,n_tokens,np_in=False):
        ...
        a_rows = n_tokens
        xqt = np.zeros(16*64*n_tokens).astype(np.float32) #todo
        xqt_g = cl.Buffer(ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=xqt)
        prg = cl.Program(ctx, f"""
        __kernel void matmul(
            __global const float *xq, __global float *xqt)
        {{
            ...
            int k = gidx0 % 64;
            xqt[i*64 + j*16*64 + k] = xq[i*{a_rows}*64 + j*64 + k];
        }}
        """).build()
        knl = prg.matmul
        g = a_rows*16*64
