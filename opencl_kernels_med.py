...
def matvec3(h_g,weight2_g,temperature,res_g):
    rows = 1024
    cols = 50257
    prg = cl.Program(ctx, f"""
    __kernel void matvec(
...
def kernel_4(a_g_2,b_g_2,b_s,c_g,d_g,f,g,start_pos,bias_g,\
    ...
    ls = 256
    zeros = np.zeros(16*(start_pos+1)).astype(np.float32)
    seg = int(dim / ls) #todo
    seg3 = math.ceil(16*(start_pos+1)*(start_pos+1) / ls)
    h = np.zeros(1024).astype(np.float32)
    ...
    {{
        ...
        __attribute__ ((aligned (16))) __local float mean;
        __attribute__ ((aligned (16))) __local float tempb3[1024*4];
        __attribute__ ((aligned (16))) __local float tempb4[1024];
        __attribute__ ((aligned (16))) __local float h_temp[1024];
        __attribute__ ((aligned (16))) __local float h[1024];
        int lidx0 = get_local_id(0);
        for(int i = 0; i < {seg}; i++) {{
            a[lidx0*{seg} + i] = a2[{b_s*1024} + lidx0*{seg} + i] + b2[lidx0*{seg} + i + {start_pos*1024}];   
        }}
        for(int r = 0; r < 24; r++) {{
        barrier(CLK_LOCAL_MEM_FENCE);  
        float total = 0;
        ...
        for(int i = 0; i < {int(dim*4 / ls)}; i++) {{
            tempb3[i + lidx0*{int(dim*4 / ls)}] = bias3[1024*4*r + i + lidx0*{int(dim*4 / ls)}];
            for(int j = 0; j < {dim}; j++) {{
                tempb3[i + lidx0*{int(dim*4 / ls)}] += ((h[j] * weight2[1024*r + j]) / mean + bias2[1024*r + j]) * weight3[1024*1024*4*r + (i + lidx0*{int(dim*4 / ls)})*{dim} + j];
            }}
        ...
        barrier(CLK_LOCAL_MEM_FENCE);  
        for(int i = 0; i < {math.ceil(1024 / ls)}; i++) {{ //todo because there's 2 now...now 12..bias4 shape
            tempb4[lidx0 + i*{ls}] = bias4[1024*r + lidx0 + i*{ls}];
            for(int j = 0; j < {dim*4}; j++) {{
                tempb4[lidx0 + i*{ls}] += tempb3[j] * weight4[1024*4*1024*r + lidx0 + i*{ls} + j*{dim}];
            }}
