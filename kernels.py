...
class Kernels:
    ...
    def kernel_1(self,h_g,weight_g,bias_g,weight2_g,temperature,random_num):
        ...
        {{
            ...
            float total = 0;
            for(int i = lidx0; i < {self.dim}; i+={ls}) {{
                total += h[i];
            }}
            temp[lidx0] = total;
            {barrier[self.d]}
    ...
    def kernel_3(self,x_g,weight_g,bias_g,attn_weight_g,attn_bias_g,new_cache_g\
        ...
        {{
            ...
            int r = gidx0 / {ls}; 
            temp2[r] = 0;
            for(int i = {self.dim}*r + lidx0; i < {self.dim}*r + {self.dim}; i+={ls}) {{
                x[i] = x_in[i];
                temp2[r] += x[i];
            }}
            temp[lidx0] = temp2[r];
        ...
        {{
            ...
            {barrier[self.d]}
            for(int i = lidx0; i < {self.dim}; i+={ls}) {{
                x[i + {(n_tokens - 1)*self.dim}] -= mean;
            }}
            {barrier[self.d]}
            total = 0;
            for(int i = lidx0; i < {self.dim}; i+={ls}) {{
                total += pow(x[i + {(n_tokens - 1)*self.dim}],2);
            }}
            temp[lidx0] = total;
            {barrier[self.d]}
            if(lidx0==0) {{
            ...
            {barrier[self.d]}
            for(int i = lidx0; i < {self.dim}; i+={ls}) {{
                x[i] = (x[i + {(n_tokens - 1)*self.dim}] * ln_f_weight[i]) / mean + ln_f_bias[i];
            }}
        }}    
    ...
    def kernel_0(self,a_g,c_g,d_g,e_g,xqkv_g,keys_values_g,attn_c_proj_weight_g,bias_g,\
        ...
        {{
            ...
            {barrier[self.d]}
            for(int j = lidx0; j < {self.dim}; j+={ls}) {{ 
                float acc0 = 0;
                for(int i = 0; i < {start_pos+1}; i++) {{
